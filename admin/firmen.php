	...
	<div class="wrapper_painel site-flex-content">	
		<div class="in_wrapper">
			...
			if(!empty($_GET['idEmpresa'])){
			    $id = $_GET['idEmpresa'];
			    $sth2 = $pdo->prepare("SELECT * FROM empresas JOIN cidades ON cid = cid_id JOIN estados ON eid = esd_id WHERE empresa = :empresa");
			    $sth2->bindValue(':empresa', $id, PDO::PARAM_INT);
			    $sth2->execute();
			    $data = $sth2->fetch(PDO::FETCH_OBJ);

			    echo "<fieldset class='listing'>";
			    echo "<h1 class='rouge'>$data->empresa</h1>";
			    echo "
			    	<p class=conteudo>
			    		Creat :: ".strftime('%m/%d/%Y %H:%M:%S', $data->data)."<br>
			    		Email :: $data->email<br>
			    	</p>";
			    echo "<p class=mais><a href='?edit=$data->empresa&uid=$data->id'><i class='icon icon-link'></i> editar</a> > user</p>";
			    echo "</fieldset>";
			}
